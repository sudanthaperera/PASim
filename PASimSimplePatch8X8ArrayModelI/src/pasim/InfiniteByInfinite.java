...
public class InfiniteByInfinite implements Runnable {
    ...
    public InfiniteByInfinite(Cell c){
    ...
    public void start(){
        t = new Thread(this);
        t.setName("Infinite-By-Infinite");
        t.start();
    }
    ...
    private void defineSources(){
       vs = new VoltageSource[2];
        ...
        vs[0] = new VoltageSource(0, 7.0e-3, -1.5e-3, 0, 7.0e-3, 0, c);
        vs[0].setDirection(Constants.ZP);
        vs[0].setResistance(50.0);
        vs[0].setMagnitude(1.0);
        
        vs[1] = new VoltageSource(7.0e-3, 0, -1.5e-3, 7.0e-3, 0, 0, c);
    ...
    private void timeMarchingLoop(){
        ...
        for (int timeIndex = 0; timeIndex < timeSteps; timeIndex++){
            ...
            if(timeIndex%(timeSteps/100) == 0){
                ...
                System.out.printf("****Infinite-By-Infinite - %d %% Done! ****  %d Seconds Remaining\n" , (100*timeIndex/timeSteps) , taskTimeMs*(timeSteps-timeIndex)/1000 );
            }
        }
    }

...
public class Common {
    ...
    public static void copyIntMetrix(int[][] oraginal, double[][] clone){
        for(int i=0;i<oraginal.length;i++){
            for(int j=0;j<oraginal[0].length;j++){
                clone[i][j] = oraginal[i][j];
            }
        }
    }
    ...
    public static int[] genInt1DArray(int nx,int initValue){
        nx = Math.abs(nx);
